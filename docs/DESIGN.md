...
- 定期実行では実際のGmail APIを呼び出し、SlackとGmailへ通知する。
- 応募用MVPでは、本人がブラウザから操作する画面と画面認証を公開しない。
...
本人用の手動確認画面を追加する場合のURLまたは認証による切り替え方法は未決定。
...
本人用の手動確認は応募用MVPに含めない。追加する場合も補助機能とし、`scheduled_checks`へ保存せず、25日の実行要否へ影響させない。
...
- Gmail APIの一時エラーを最大2回再試行し、認証エラーは再試行しないテスト
- Gmail検索の一時的な403を再試行するテスト
- Gmail通知のPOSTは一時エラーでも再試行しないテスト
- Gmail認証エラー時にGmail送信を省略し、Slackへ再接続を通知するテスト
- Gmail通知の送信時に認証エラーになった場合もSlackへ再接続を通知するテスト
- Route Handlerの認証成功、認証失敗、処理例外のテスト
- データベースへの実行予約、重複防止、取得、更新失敗のテスト
- Slack未設定、成功、失敗のテスト
- Gmail通知成功、失敗のテスト
- 一方の通知失敗が他方へ影響しないテスト
